...
- [Go+Redis compose with health checks](Docker/manifests/2026-06-28-go-redis-compose-healthchecks.yaml) — Multi-service compose stack with health checks and network configuration.
- [Deploy keys vs PATs for CI/CD](GitHub/docs/how-i-wired-deploy-keys-vs-fine-grained-pats-for-cicd.md) — Notes on configuring deploy keys and fine-grained personal access tokens for CI/CD access.
- [Provision repository with GitHub API](GitHub/scripts/provision-repo-with-api.py) — Python script to create, configure, and secure a repository via the GitHub API.
- [Build mount vs COPY caching](Docker/docs/docker-build-mount-vs-copy-caching.md) — Comparison of dependency caching strategies in Docker builds.
- [Trivy container image scan](Trivy/scripts/2026-06-26-scanned-first-container-image.sh) — Script to scan a container image and parse JSON results.
