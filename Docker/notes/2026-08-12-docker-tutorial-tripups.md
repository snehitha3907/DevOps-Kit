...
The tutorial starts by cloning a small sample app and building it:
...
The `-d` flag runs the container in the background, and `-p 127.0.0.1:3000:3000` maps my port 3000 to the container's port 3000. I bound only to `127.0.0.1` so nothing outside my machine can reach it.
...
- **`Dockerfile` vs `dockerfile` casing.** On my Linux filesystem the lowercase `dockerfile` worked because Docker accepts both, but I renamed it to `Dockerfile` for consistency when I shared the repo — renaming mid-tutorial added nothing but confusion.
- **Changes not showing up.** I edited the app source and rebuilt, but the old behavior stuck around. I'd forgotten I was still running the *previous* container on port 3000. The fix was `docker ps`, then `docker rm -f <name>` to stop and remove it, then rebuild and run again. Running a second `docker run` on the same port just errors with "port is already allocated".
